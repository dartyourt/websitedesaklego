...
if (isset($_POST['submit'])) {
    ...
    if ($query) {
        echo "<script>alert('Berita baru berhasil diterbitkan!'); window.location='berita.php';</script>";
        exit;
    } else {
...
<head>
    ...
    <title>Tambah Berita - Admin CMS</title>
    ...
    <header class="bg-[#165f36] text-white shadow-md border-b-4 border-amber-500">
        <div class="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="font-bold text-lg leading-tight">Terbitkan Berita / Agenda Baru</h1>
                    <p class="text-[11px] text-amber-300">Editor Visual Otomatis - Tanpa Kode HTML</p>
                </div>
            </div>
            <a href="../berita.php" target="_blank" class="bg-amber-500 text-slate-900 text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-1.5 shadow">
        ...
        <div class="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
            ...
            <form action="" method="post" enctype="multipart/form-data" class="space-y-6" onsubmit="tinymce.triggerSave()">
                ...
                <div>
                    ...
                    <input type="text" name="judul" class="w-full border border-slate-300 px-4 py-3 rounded-xl focus:ring-2 focus:ring-emerald-600 text-base font-bold" placeholder="Contoh: Pembagian Bibit Tanaman Gratis untuk Desa Klego" required>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="tanggal" value="<?= date('Y-m-d') ?>" class="w-full border border-slate-300 px-4 py-2.5 rounded-xl text-sm" required>
                    </div>

                    <div>
                        <label class="block mb-2 font-bold text-xs uppercase text-slate-700">Foto Sampul Utama (Cover) <span class="text-rose-500">*</span></label>
                        <input type="file" name="foto" class="w-full px-3 py-2 border border-slate-300 rounded-xl text-xs text-slate-600 file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:font-bold file:bg-emerald-50 file:text-emerald-800" accept="image/*" required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="editorIsiBerita" name="isi" rows="12" class="w-full border rounded-xl p-4" placeholder="Tulis berita desa di sini..."></textarea>
