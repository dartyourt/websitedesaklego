...
/* =====================================================
   HITUNG TOTAL SURAT
===================================================== */
$qTotal = mysqli_query($koneksi, "SELECT COUNT(*) total FROM sktm");
...
if (isset($_POST['preview']) && !empty($_POST['nik'])) {
    ...
    $namaPejabat  = $sekdes['nama'] ?? '';
        ...
        INSERT INTO sktm
        (nik, nomor_surat, keperluan, keterangan_lain, pejabat, tgl_surat, ttd_camat)
        VALUES ('$nik','$nomor','$keperluan','$ket_lain','$pejabat','$tgl','$ttdCamat')
        ");
...
<h1 class="text-xl font-bold text-center mb-4">📄 SURAT KETERANGAN TIDAK MAMPU</h1>
...
<textarea name="keterangan_lain" class="border p-2 rounded w-full" placeholder=""></textarea>
...
<?php if ($modePreview && $dataPenduduk) {
    ...
    $tanggalSurat = !empty($dataPenduduk['tgl_surat'])
...
<h3 class="text-center font-bold underline mb-2">SURAT KETERANGAN TIDAK MAMPU</h3>
...
<tr><td width="5%"></td><td width="30%">Nama Lengkap</td><td>: <?= htmlspecialchars($dataPenduduk['NAMA_LGKP']) ?></td></tr>
<tr><td width="5%"></td><td width="30%">Jenis Kelamin</td><td>: <?= ($dataPenduduk['JENIS_KELAMIN'] == 'LK') ? 'LAKI-LAKI' : 'PEREMPUAN'; ?></td></tr>
<tr><td></td><td>Tempat / Tanggal Lahir</td><td>: <?= htmlspecialchars($dataPenduduk['TMPT_LAHIR']) ?>/<?= strtoupper (tglIndonesia($dataPenduduk['TGL_LAHIR'])) ?></td></tr>
<tr><td></td><td>Warga Negara</td><td>: INDONESIA</td></tr>
<tr><td></td><td>Agama</td><td>: <?= htmlspecialchars($dataPenduduk['AGAMA']) ?></td></tr>
<tr><td></td><td>NIK</td><td>: <?= htmlspecialchars($dataPenduduk['NIK']) ?></td></tr>
<tr><td></td><td>No. KK</td><td>: <?= htmlspecialchars($dataPenduduk['NO_KK']) ?></td></tr>
<tr><td></td><td>Pekerjaan</td><td>: <?= htmlspecialchars($dataPenduduk['PEKERJAAN']) ?></td></tr>
<tr><td></td><td>Alamat</td><td>: <?= htmlspecialchars($dataPenduduk['DUSUN']) ?> RT <?= htmlspecialchars($dataPenduduk['RT']) ?> RW <?= htmlspecialchars($dataPenduduk['RW']) ?>, <?= strtoupper(htmlspecialchars($APP_PROFIL['nama_desa'])) ?>, <?= strtoupper(htmlspecialchars($APP_PROFIL['kecamatan'])) ?>, <?= strtoupper(htmlspecialchars($APP_PROFIL['kabupaten'])) ?></td></tr>
</table>
<p class="indent-8 text-sm text-justify mb-6">Berdasarkan Surat Keterangan dari Ketua RT <?= htmlspecialchars($dataPenduduk['RT']) ?> <?= htmlspecialchars($dataPenduduk['DUSUN']) ?> Tanggal <?= tglIndonesia($tanggalSurat) ?> , bahwa yang bersangkutan betul warga <?= htmlspecialchars($APP_PROFIL['nama_desa']) ?> dan yang bersangkutan keadaan ekonominya <span class="font-bold">TIDAK MAMPU</span></p>
<p class="indent-8 text-sm text-justify mb-6">Surat Keterangan ini diperlukan untuk <?= strtoupper(htmlspecialchars($_POST['keperluan'] ?? $dataPenduduk['keperluan'] ?? '')) ?></p>
...
if ($pejabatDipilih == 'kepala' && $kades) {
    $jabatanTtd = $kades['jabatan'];
    $namaTtd = $namaPejabat;
} 
elseif ($pejabatDipilih == 'sekdes' && $sekdes) {
    $jabatanTtd = $sekdes['jabatan'];
    $namaTtd = $namaPejabat;
}
?>
        <!-- KIRI -->
...
<td width="50%" valign="top">
    ...
    <?php if ($ttdCamat && $camat) { ?>
        <br><br><br>
        <p style="text-align:center;">
            ( <strong><?= $camat['nama'] ?></strong> )<br>
            <?= $camat['gol'] ? $camat['gol'].'<br>' : '' ?>
            <?= $camat['nip'] ? 'NIP. '.$camat['nip'] : '' ?>
        </p>
    <?php } ?>
